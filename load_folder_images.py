...
def _decode_and_preprocess_image(image_file, shift_param = -128, rescale_param = 128, resized_image_size = [128, 128], image_channels = 1, force_to_grayscale=False):
  ...
  if(image_channels == 3 and force_to_grayscale == True):
  #   image.set_shape((resized_image_size[0], resized_image_size[1], 3))
    image = tf.image.rgb_to_grayscale(image)
    image.set_shape((resized_image_size[0], resized_image_size[1], 1))
  else:
    image.set_shape((resized_image_size[0], resized_image_size[1], image_channels))
  #image = tf.image.grayscasle_to_rgb(image, 'torgb')
  return image


def _load_images(image_file, batch_size, num_preprocess_threads, min_queue_examples, shift_param = -128, rescale_param = 128, resized_image_size = [128, 128], image_channels = 1, shuffle = True):
  
  image = _decode_and_preprocess_image(image_file, shift_param, rescale_param, resized_image_size, image_channels)
  #image = tf.image.grayscasle_to_rgb(image, 'torgb')
  images = []
  if shuffle == True:
...
def load_image_and_segmentation_from_idlist(idlist_tensor_img, idlist_tensor_seg, batch_size, num_preprocess_threads, min_queue_examples, shift_params = [-128, -0.5], rescale_params = [128, 0.5], resized_image_size = [128, 128], image_channels=1, force_to_grayscale=False, shuffle = True):
  ...
  processed_image = _decode_and_preprocess_image(image_file_image, shift_params[0], rescale_params[0], resized_image_size, image_channels, force_to_grayscale=force_to_grayscale)

  #processed_image = processed_image + tf.random_normal(processed_image.shape, stddev=0.00)
  ...
  processed_seg = _decode_and_preprocess_image(image_file_seg, shift_params[1], rescale_params[1], resized_image_size, force_to_grayscale=force_to_grayscale)
  if shuffle == True:
...
#TODO: test this, needed for comparing image features from discriminator
